...
class Ego4dTwoHead_VisionTransformer(VisionTransformer):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        self.cls_head = nn.Linear(kwargs["embed_dim"], 2)
        self.loc_head = nn.Linear(kwargs["embed_dim"], 1)

        trunc_normal_(self.cls_head.weight, std=.02)
        self.cls_head.weight.data.mul_(kwargs["init_scale"])
        self.cls_head.bias.data.mul_(kwargs["init_scale"])

        trunc_normal_(self.loc_head.weight, std=.02)
        self.loc_head.weight.data.mul_(kwargs["init_scale"])
        self.loc_head.bias.data.mul_(kwargs["init_scale"])
    ...
    def forward(self, x):
        ...
        loc = self.loc_head(x) # shape: bs, T, 2
