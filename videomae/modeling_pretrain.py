...
class MaskAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        masked_attn = self.attn_drop(masked_attn)

        x = (masked_attn @ v).transpose(1, 2).reshape(B, N, -1)
...
class PretrainMultiModalTransformer(nn.Module):
    ...
    def forward(self, rgb, flow, rgb_mask, flow_mask, all_token=False):
        ...
        if "rgb" in self.modality:
            ...
            _, N_rgb_mask, _ = rgb_pos_emd_mask.shape
        ...
        if self.modality == "rgbflow":
            ...
            # cross_rgb_hat = self.rgb_decoder(intra_flow_full, cross_flow_vis, N_rgb_mask if not all_token else 0) # [2*B, N_mask, 2 * 16 * 16]
            # cross_flow_hat = self.flow_decoder(intra_rgb_full, cross_rgb_vis, N_flow_mask if not all_token else 0) # [2*B, N_mask, 3 * 16 * 16]
            ...
            intra_rgb_hat = self.rgb_decoder(intra_rgb_full, intra_rgb_full,  N_rgb_mask if not all_token else 0) # [2*B, N_mask, 3 * 16 * 16]
            rgb_hat = intra_rgb_hat
            flow_hat = None
        
        elif self.modality == "flow":
            intra_flow_hat = self.flow_decoder(intra_flow_full, intra_flow_full, N_flow_mask if not all_token else 0) # [2*B, N_mask, 2 * 16 * 16]
            rgb_hat = None
...
class PretrainBottleneckEncoder(nn.Module):
    ...
    def forward_features(self, rgb, flow, rgb_mask, flow_mask):
        ...
        expand_bottleneck = self.bottleneck.expand(B, -1, -1).type_as(x_rgb).to(x_rgb.device)
        x = torch.cat((x_rgb_vis, expand_bottleneck, x_flow_vis), dim=1)
        # prepare mask
        n_rgb = x_rgb_vis.shape[1]
        n_flow = x_flow_vis.shape[1]
        up_mask = torch.cat((torch.ones(n_rgb,n_rgb), torch.zeros(n_rgb, self.num_bottleneck+n_flow)), dim=1)
        mid_mask = torch.ones(self.num_bottleneck, n_rgb + self.num_bottleneck + n_flow)
        down_mask = torch.cat((torch.zeros(n_flow,n_rgb), torch.ones(n_flow, n_flow+self.num_bottleneck)), dim=1)
        attn_mask = torch.cat((up_mask, mid_mask, down_mask), dim=0).to(x_rgb.device)

        for blk in self.blocks:
            x = blk(x, attn_mask)

        return x[:, :n_rgb], x[:, n_rgb:n_rgb+self.num_bottleneck], x[:, n_rgb+self.num_bottleneck: ]

    def forward(self, rgb, flow, rgb_mask, flow_mask):
        x = self.forward_features(rgb, flow, rgb_mask, flow_mask)

        return x


class PretrainBottleneckTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = PretrainBottleneckEncoder(
                                                   # default value
            rgb_in_chans = rgb_in_chans,           # 3
            flow_in_chans = flow_in_chans,         # 2
            rgb_num_frames = rgb_num_frames,       # 16
    ...
    def forward(self, rgb, flow, rgb_mask, flow_mask, all_token=False):
        ...
        x = self.encoder(rgb, flow, rgb_mask, flow_mask) 
        x1, bottleneck, x2 = x # [B, 2*N_vis, C_e]

        # print(x1.shape, bottleneck.shape, x2.shape)
        n_rgb = x1.shape[1]

        x = self.encoder_to_decoder(torch.cat(x, dim=1))
        x1, bottleneck, x2  = x[:, :n_rgb], x[:, n_rgb:n_rgb+self.num_bottleneck], x[:, n_rgb+self.num_bottleneck:]

        B, _, C = x1.shape
        expand_pos_embed = self.rgb_pos_embed.expand(B, -1, -1).type_as(x1).to(x1.device).clone().detach()
        rgb_pos_emd_vis = expand_pos_embed[~rgb_mask].reshape(B, -1, C)
        # print(rgb_pos_emd_vis.shape)
        rgb_pos_emd_mask = expand_pos_embed[rgb_mask].reshape(B, -1, C)
        rgb_full = torch.cat([x1 + rgb_pos_emd_vis, self.rgb_mask_token + rgb_pos_emd_mask], dim=1)    # [2*B, N, C_d]
        _, N_rgb_mask, _ = rgb_pos_emd_mask.shape

        expand_pos_embed = self.flow_pos_embed.expand(B, -1, -1).type_as(x1).to(x1.device).clone().detach()
        flow_pos_emd_vis = expand_pos_embed[~flow_mask].reshape(B, -1, C)
        flow_pos_emd_mask = expand_pos_embed[flow_mask].reshape(B, -1, C)
        flow_full = torch.cat([x2 + flow_pos_emd_vis, self.flow_mask_token + flow_pos_emd_mask], dim=1) # [2*B, N, C_d]
        _, N_flow_mask, _ = flow_pos_emd_mask.shape

        rgb_hat = self.rgb_decoder(rgb_full, bottleneck, N_rgb_mask if not all_token else 0) # [2*B, N_mask, 2 * 16 * 16]
        flow_hat = self.flow_decoder(flow_full, bottleneck, N_flow_mask if not all_token else 0) # [2*B, N_mask, 3 * 16 * 16]


        return rgb_hat, flow_hat


# class PretrainMultiModalTransformer_v2(nn.Module):
...
#         if "rgb" in modality:
#             self.rgb_decoder = PretrainVisionTransformerDecoder(
...
#         if "flow" in modality:
#             self.flow_decoder = PretrainVisionTransformerDecoder(
...
@register_model
def pretrain_videomae_bottleneck_base_patch16_224(pretrained=False, **kwargs):
    model = PretrainBottleneckTransformer(
        rgb_in_chans=3, 
        flow_in_chans=2,
