...
class StateChangeDetectionAndKeyframeLocalisation(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        assert mode in [
            ...
            'val',
            'test'
        ], "Split `{}` not supported for Keyframe detection.".format(mode) # = assert 为FALSE时，报Split ‘mode’ not supported for Keyframe detection.
        self.mode = mode
        self.cfg = cfg
        self.ann_path = os.path.join(cfg.DATA.ANN_PATH, f'fho_oscc-pnr_{self.mode}.json') #f 是格式化字符串，替换{}中内容
    ...
    def _construct_loader(self): #加载annotation
        self.package = dict()
        self.ann_data = json.load(open(self.ann_path, 'r'))["clips"]
        for count, value in enumerate(
            tqdm(self.ann_data, desc='Preparing data') #进度条
        ):
            # [1]

            if self.cfg.DATA.READ_BY_CLIPS: #设置为TRUE
                try:
            ...
            #print(self.mode)
            if self.mode in ['train', 'val']:  #判断模式
    ...
    def __getitem__(self, index): #视频信息预处理
    ...
    def _extract_clip_frames(self, info):
        ...
        if os.path.isdir(clip_save_path):
            ...
            if num_frames < (clip_end_frame - clip_start_frame):
                ...
                os.system(f'rm -rf {clip_save_path}')
            else:
                return None
        ...
        num_saved_frames = 0
        for frame, frame_count in zip(frames, frames_list):  #格式化视频h，w
